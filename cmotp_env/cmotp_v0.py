...
class CMOTP(gym.Env):
    ...
    def __init__(self, version=1, cooperation_level=0.0):
        '''
        ...
        self.c.DIM += [1] # TODO: this is needed for stable-baselines but breaks original NUI implementation
        ...
        self.observation_space = gym.spaces.Box(low=0, high=3, dtype=np.uint8, shape=[2]+self.c.DIM)
        self.action_space = gym.spaces.MultiDiscrete([self.c.ACTIONS]*2)
        # self.action_space = gym.spaces.Discrete(self.c.ACTIONS**2)
    ...
    def render(self, mode='human'):
        ...
        screen_width = screen_height = r*16

        if self.viewer is None:
            self.viewer = rendering.Viewer(screen_width, screen_height)
            self.viewer.set_bounds(-r//2, screen_width + 0.5*r, -r//2, screen_height + 0.5*r)


        state = self.getNoisyState()
        for x in range(len(state)):
            for y in range(len(state[0])):
                vertices = np.array([(x-0.5,y-0.5),(x+0.5,y-0.5),(x+0.5,y+0.5),(x-0.5,y+0.5)])*r
                c = state[15-y,x]/256
                self.viewer.draw_polygon(vertices, color=(c,c,c))
    ...
    def step(self, actions):
        ...
        try:
            actions[1]
        except IndexError: # only a single action
            actions = [actions//self.c.ACTIONS, actions%self.c.ACTIONS]
        ...
        return observations, rewards, self.terminal(), dict()
    ...
    def goodsDelivered(self):
        ...
        '''
        for (dropX, dropY, reward) in self.c.DZONES:
    ...
    def getNoisyState(self, agent=0):
        '''
        Method returns noisy state. Modifies the observation so each agent sees
        itself as agent 0
        '''
        state = self.s_t
        if agent!=0:
            state[self.agents_y[agent]][self.agents_x[agent]]=self.c.AGENTS[0]
            state[self.agents_y[0]][self.agents_x[0]]=self.c.AGENTS[agent]
        return state + (self.c.NOISE * self.ones *\
    ...
    def getObservations(self):
        ...
        for i in range(self.n_agents):
            # Store observation
            observations.append(np.copy(self.getNoisyState(i)))
        return observations
    ...
    def moveAgent(self, id, targetx, targety):
        ...
        '''
        self.s_t[self.agents_y[id]][self.agents_x[id]] = 0
